...
export default function Calendar({
            ...
            >
                <div>
                    ...
                    {selectedTask.endTime.date
                        ...
                        .replace(/,/g, '')}
                    <br />
                    <div className="CalendarTaskTitle">{selectedTask.title}</div>
                    <br />
                    <div className="CalendarTaskDescription">{selectedTask.description}</div>
                </div>
            </div>
        );
        return html;
    ...
    function insertCalendarEvent(currentDay: Date) {
        ...
        for (let i = 0; i < todaysTasks.length; i++) {
            ...
            const hoursSincePreviousTask = getHoursBetween(previousTaskStartTime, startTime);

            const heightOfFillerDiv: number =
                hoursSincePreviousTask * hourHeightInPixels +
                hoursSincePreviousTask * heightOfCalendarEventContainerMarginTopPlusBottom;
            ...
            if (taskCrossesMidnight(todaysTasks[i])) {
                if (thisIsTheFirstDayOfTheTask(startTime, currentDay)) {
                    taskHeight =
                        ...
                        (hourHeightInPixels + heightOfCalendarEventContainerMarginTopPlusBottom);
                    ...
                    taskHeight = 24 * (hourHeightInPixels + heightOfCalendarEventContainerMarginTopPlusBottom);
                }
            ...
            html.push(
                ...
                >
                    <div className="CalendarEventTitle">
                        {duration} <br />
                        <div className="CalendarTaskTitle">{title}</div>
                        <br />
                        <div className="CalendarTaskDescription">{description}</div>
                    </div>
                </div>
            );
    ...
    function thisIsTheFirstDayOfTheTask(startTime: DateAndTime, currentDay: Date): boolean {
    ...
    function thisIsTheLastDayOfTheTask(endTime: DateAndTime, currentDay: Date): boolean {
    ...
    function getSortedTasksForThisDate(date: Date) {
        ...
        return tasksThisDay.slice().sort((a, b) => a.startTime.date.getTime() - b.startTime.date.getTime());
    }
